...
bool	try_add_ambient_light_to_world(char** attributes, t_world* world)
{
	...
	bool is_invalid_value = try_atof(attributes[AMBIENT_ATTRIBUTE_RATIO_IN_RANGE], &world->ambient_light.brightness) == false \
...
t_world* world, t_canvas* canvas)
{
	...
	bool is_invalid_value = try_parse_vector3(attributes[CAMERA_ATTRIBUTE_COORDINATES], &world->camera.coordinates) == false \
...
bool	try_add_light_to_world(char** attributes, t_world* world)
{
	...
	is_invalid_value = try_parse_vector3(attributes[LIGHT_ATTRIBUTE_COORDINATES], &light.coordinates) == false \

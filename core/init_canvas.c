...
/*   Updated: 2025/02/26 05:28:02 by sejjeong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_canvas(t_canvas *out_canvas)
{
	...
	mlx_get_screen_size(out_canvas->xvar, (int *)&out_canvas->screen.width, (int *)&out_canvas->screen.height);
	out_canvas->screen.height = get_min(out_canvas->screen.height, 1024);
	out_canvas->screen.width = get_min(out_canvas->screen.width, 1024);
	out_canvas->win = mlx_new_window(out_canvas->xvar, out_canvas->screen.width, out_canvas->screen.height, "miniRT");
...
{
	t_img* img = mlx_new_image(pa_canvas->xvar, pa_canvas->screen.width, pa_canvas->screen.height);
